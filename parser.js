...
function renderAST(node) {
  ...
  // If it's an array, wrap children directly
  ...
  // Recursively print properties (skip the "type" field itself)
  html += `<ul>`;
...
document.getElementById('parseBtn').addEventListener('click', () => {
  ...
  astOutput.innerHTML = `<ul>${renderAST(ast)}</ul>`;
  attachTreeInteractivity(astOutput);   // make it interactive
});